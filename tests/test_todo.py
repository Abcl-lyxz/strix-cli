...
@pytest.fixture
def todo_store(tmp_path: Path) -> JsonArtifactStore:
    return JsonArtifactStore(tmp_path / "todos.json")


async def _create(
    store: JsonArtifactStore, todos: list[Any], agent_id: str = "root"
) -> dict[str, Any]:
    scan_context = SimpleNamespace(artifact_store=lambda _name: store)
    ctx = ToolContext(
        context={"agent_id": agent_id, "scan_context": scan_context},
...
@pytest.mark.asyncio
async def test_one_bad_priority_no_longer_discards_the_batch(
...
@pytest.mark.asyncio
async def test_duplicate_titles_within_a_batch_are_skipped(
    todo_store: JsonArtifactStore,
) -> None:
    result = await _create(
        todo_store,
...
@pytest.mark.asyncio
async def test_a_title_already_on_the_list_is_not_created_again(
    todo_store: JsonArtifactStore,
) -> None:
...
@pytest.mark.asyncio
async def test_non_string_priority_does_not_fail_the_batch(
    todo_store: JsonArtifactStore,
) -> None:
    result = await _create(
        todo_store,
