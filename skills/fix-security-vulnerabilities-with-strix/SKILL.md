...
description: Remediate local Strix findings and verify each fix by rerunning a scoped scan.
license: Apache-2.0
metadata:
  author: usestrix
...
After fixing, re-scan scoped to the fixed area and confirm the finding is gone. Use the same local scope:
