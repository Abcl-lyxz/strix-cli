...
Versions 1.7.0 and 1.8.0 can incorrectly look up an upstream PyPI version and then report a missing GitHub manifest. For a Windows uv installation, bootstrap the fixed updater once with:

```powershell
uv tool install --force https://github.com/Abcl-lyxz/strix-cli/releases/download/v1.9.0/strix_agent-1.9.0-py3-none-win_amd64.whl
strix --version
```
