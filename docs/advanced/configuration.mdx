...
Configure Strix from its interactive TUI, with environment variables, or with a
config file. Run `strix`, type `/config` to inspect the active non-secret values,
and use `/model`, `/apikey`, `/baseurl`, or `/reasoning` to change them. Type `/`
for the complete searchable command palette.

The TUI masks API keys and stores reusable credentials in Windows Credential
Manager, macOS Keychain, or Linux Secret Service under the `strix-cli`
namespace. `~/.strix/cli-config.json` contains only non-secret settings and
opaque credential references. It never sends a stored key back to the terminal
client. Environment variables take precedence and remain process-only.
...
<ParamField path="STRIX_ROUTE_OUTAGE_TIMEOUT" default="600" type="integer">
  Maximum seconds a headless run waits for a route to recover before it
  checkpoints and exits nonzero. Interactive runs remain parked until a route
  recovers or the user changes configuration.
</ParamField>

<ParamField path="STRIX_ROUTES_FILE" type="string">
  Headless route-pool JSON file. Secret values are rejected; use `api_key_env`
  and `headers_env` references to process environment variables.
</ParamField>
...
Application logs and scan records stay on this computer. Outbound usage analytics and remote trace export have been removed.
...
Do not add literal keys or tokens to this file. Use `strix routes key <name>`
or an environment variable. See [Routes, recovery, and storage](/advanced/resilience-and-storage)
for the route schema and migration behavior.
